...
@Component({
  selector: 'app-orbit-card-promoter-5',
  templateUrl: './orbit-card-promoter-5.component.html',
  styleUrls: ['./orbit-card-promoter-5.component.scss']
})
export class OrbitCardPromoter5Component implements OnInit {
  ...
  constructor(private orbitService: Orbit5PromoterService) {}
  ...
  applyInvestments(): void {
    if (this.users.length < 5) {
      ...
      this.errorMessage = `Se requieren 5 usuarios. Actualmente hay ${this.users.length}`;
      return;
    }
    ...
    console.log('Total de Órbita 5 enviado:', this.totalInvestment);
  ...
  private calculateTotalInvestment(): void {
    ...
    console.log('Inversiones Órbita 5:', this.users.map(u => ({
      nombre: u.name,
      monto: u.investment
    })));
